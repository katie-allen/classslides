...
We will use `leaflet` to create interactive maps of the cumulative confirmed cases of COVID-19. First we will plot the cities with the most confirmed cases.
...
covid_top_cities <- covid %>% 
  select(confirmed, UID:Combined_Key) %>% 
  slice_______(order_by = _______, n = _____)
head(covid_top_cities)
```
...
Let's add labels to the markers so we know the name of the city and the cumulative # of confirmed cases. In addition to blanks, this code has 3 errors in it. One will prevent the code from running, the others are formatting mistakes. Can you find and correct them?
